...
// Load site config if present (Figma Make environment), otherwise use defaults.
let siteConfiguration: Record<string, unknown> = {}
try {
  const raw = fs.readFileSync(new URL('./.figma/make/site.json', import.meta.url), 'utf-8')
  siteConfiguration = JSON.parse(raw)
} catch {
  siteConfiguration = { title: 'Anderside', description: '', robots: { index: true } }
}
